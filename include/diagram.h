...
public:
	...
	const std::vector<std::vector<Entry>>& get_data() 
	{
		return data;
	}
	...
	geom::PolygonalCurve generate_curve() const
	{
		...
		for (const auto& absolute_index : indices)
		{
			...
			float z = std::count(lifted.begin(), lifted.end(), absolute_index) ? lift_amount : 0.0f;

			curve.push_vertex({ x, y, z });
